    ...
    try {
        ...
        if (!hasSanctumBadge) {
            updatedWitch = await Witch.findByIdAndUpdate(
                witch._id, //The Oracles Absolute Truth and the witch's true soul

                // _id VS username: 
                // a witch might be able to change their username but their _id will NEVER change
                // MongoDb indexes _id fields automatically so searching the DB by _id is the fastest way to find a document
                // _id is standared practice in backend architecture (REST APIs), the routes that target a specific user directly (router.get('_id), router.put('/:id), router.delete('/:id)) expect that unique database ID in the URL

                //Username
                // use username in /login /register and /save-progress blocks bc they ar heavily interacting with human input or frontend local storage.
                //LOGIN/REGISTER: humans don't memorise random 24 charcter strings, they memorise 'GoblinQueen'. So we have to search the database using the username they type in the login box
                // AUTO-SAVE: in the front end the logged in witch's username is saved in localStorage. When the auto-save triggers, it grabs that easy-to-read name from local storage and sends it to the backend. The backend then says "Oh Great Oracle, find the witch whose chosen name is 'GoblinQueen" and update them
                {
...
router.put('/save-progress', async (req, res) => {
    ...
    try {
        ...
        //consukt oracle to find the witch
        const witch = await Witch.findOne({ username });
        ...
        const hasOracleBadge = witch.achievements?.some(badge => badge.title === "Oracle's Eye");

        let updatedWitch;

        if (!hasOracleBadge) {
            //find the witch by their coven name and update their stats
            updatedWitch = await Witch.findOneAndUpdate(
                ...
                { username: username },
                {
                    $set: {
                        ...
                        hasMidnightElixir: hasMidnightElixir
                    },
                    $push: {
                        achievements: {

                            title: "Oracle's Eye",
                            description: "The Oracle realises your progress and welcomes you with this mark",
                            longDescription: "The Oracle sees all, and by recognizing your dedication and progress, it bestows upon you the Oracle's Eye. This badge symbolizes your connection to the mystical forces that guide our coven. Wear it with pride, for it signifies your commitment to the craft and your place in our sacred circle.",
                            iconUrl: "./badges/oracleEye-badge.png", //maps to public folder
                            dateEarned: new Date()
                        }
                    }
                },
...
console.log(`👁️ The Oracle's Eye has marked ${username} for their first cloud save!`)
...
res.status(200).json({
    ...
    res.status(500).json({ message: `The Oracle told you to beware: ${error.message}` });
}
});
